  ...
  ///////////////////////// TO-DO (3) //////////////////////////////
   ...
   /*for(auto i=shoppingCarts.cbegin();i!=shoppingCarts.cend();++i){
      ...
      */
      ...
      //Print the customer's (name) cart contains
      std::cout<<name<<" 's cart contains:\n";
      double amountDue = 0.0;   //initialize amount due to 0

     //For each book in the customer's shopping cart
    /* for(auto i=cart.cbegin();i!=cart.cend();++i){
       const auto & isbn = i-> first;
       const auto & book= i-> second;
    */
    for(const auto &[isbn,book]:cart){
       auto fullItemDescription = worldWideBookDatabase.find(isbn);

       std::cout<<'\t';
      if (fullItemDescription != nullptr){

        //Print the full description on the receipt
        std::cout<< fullItemDescription<<std::endl;
        amountDue += fullItemDescription->price(); //Add the book's price to the amount due

        //If the book is something the store sells (the item is in the store's inventory)
      ...
      //If the book is not found in the database of all books in the world, indicate on the receipt it's free of charge
        else{
          std::cout << "\"" <<isbn<< "\", \""  <<book.title()<< "is not found, the book is free!\n";
        }

    //Print the total amount due on the receipt
    std::cout << "\n";
    std::cout<<'\t'<<std::string(25,'-')<<'\n'
    <<'\t'<< "Total $" << amountDue << std::endl;
  }
}
  /////////////////////// END-TO-DO (3) ////////////////////////////
...
std::size_t lineItemCount = 0;

//For each book sold today
for(const auto isbn: todaysSales){
  auto inventoryRecord = _inventoryDB.find(isbn);

  //If the book is not in the store's inventory or if the number of books on hand has fallen below the re-order threshold
  if( inventoryRecord == _inventoryDB.end() || inventoryRecord< REORDER_THRESHOLD){
    std:: cout << '\n' << std::setw(2) << ++lineItemCount << ": ";
    auto fullItemDescription = worldWideBookDatabase.find(isbn);
    //If the book is not in the database of all books in the world,
     if( fullItemDescription == nullptr){
       //display just the ISBN
       std::cout<<'" '<<isbn<< "\ " << std::endl;
     }
     else{
       //display the book's full description
       std::cout<<'{'<< fullItemDescription<< "}"<<std::endl;
     }

     if(inventoryRecord == _inventoryDB.end()){
       //If the book is not in the store's inventory
       std::cout<<"We apologize but"<< fullItemDescription<<" "<<"is no longer sold in this store and will not be re-ordered"<<std::endl;
     }
     else{
       std::cout <<"\n only"<<inventoryRecord->second <<"remain in stock which is "
       <<REORDER_THRESHOLD - inventoryRecord->second << "unit(s) below reorder threshold ("
       <<REORDER_THRESHOLD << "), re-ordering" << LOT_COUNT << "more\n";

       inventoryRecord-> second += LOT_COUNT;
     }
      ...
      //Reset the list of book sold today so the list can be reused again later
      todaysSales.clear() ;
  /////////////////////// END-TO-DO (4) ////////////////////////////
}
