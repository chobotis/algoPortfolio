...
*/
    ...
    BookList newBook =        {{"Land of the Lustrous", "Haruko Ichikawa"               },
                              {"Creativity Inc.", "Ed Catmull"          },
                              {"Art of Space X"                                  },
                              {"Shadow Over Innsmouth", "HP Lovecraft" },
                              {"Labyrinth of the Mind's Eye", "V.S. Rasthmantha"                   }};

   //display
   std::cout<<"Here we have in the library: "<<newBook<<std::endl;

    //update

   newBook.moveToTop({"Art of Space X"});

   std::cout<<"The updated library is: "<<newBook<<std::endl;

   //new BookList class
   BookList halo =           {{"Primordium", "Greg Bear"               },
                             {"Silentium", "Greg Bear"          },
                             {"Cryptum", "Greg Bear"                   }};


   if( newBook != halo ) newBook += halo;


   std::cout<<"The combined library is: "<< newBook <<std::endl;

    newBook.insert({"Before the Flood", "Young-In Kim"}, newBook.find({"Land of the Lustrous"}) );
    newBook.remove({"Creativity Inc.", "Ed Catmull"});
