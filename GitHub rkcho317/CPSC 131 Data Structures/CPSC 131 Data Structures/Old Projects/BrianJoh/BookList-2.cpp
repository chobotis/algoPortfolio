...
{
  ///////////////////////// TO-DO (2) //////////////////////////////
  for (auto book = rhs.begin(); book != rhs.end(); book++)
  {
...
std::size_t BookList::find( const Book & book ) const
{
  ...
  ///////////////////////// TO-DO (5) //////////////////////////////
  std::size_t x = 0;
while(x != _books_array_size){ // Will keep looping untill it has reached the end of the array
  if (book == _books_array[x]) return x; // If x points to the correct book it will return the currect position
  x++;
}
return _books_array_size; // If the book isnt in the list return the current size of the array
...
void BookList::insert( const Book & book, std::size_t offsetFromTop )       // insert new book at offsetFromTop, which places it before the current book at offsetFromTop
{
  ...
  ///////////////////////// TO-DO (6) //////////////////////////////
  if( find( book ) != size() ) return; // This will check to see if the book is already in the list, the find function will return the index of the book if it is in the list, so the function will return without doing anything because the book was already in the list.
